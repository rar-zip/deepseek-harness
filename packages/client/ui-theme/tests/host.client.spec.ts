...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    expect(ctx.settings.get(ns)).toEqual({ preference: DEFAULT_PREFERENCE, importedThemes: [] })
    await ctx.settings.update(ns, { preference: 'dark' })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'dark', importedThemes: [] })
    // Any string is a valid durable preference: built-in or a registered theme id.
    await ctx.settings.update(ns, { preference: 'sepia' })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'sepia', importedThemes: [] })
    await fiber.dispose()
